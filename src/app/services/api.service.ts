...
export class ApiService {
  ...
  public searchBooks(query: string, startIndex: number=0, orderBy:string='relevance', filter:string='', langRestrict:string=''): Observable<{books: BookModel[], totalItems: number}> {
    ...
    const queryReq = filter === '' ?
      `https://www.googleapis.com/books/v1/volumes?q=${query}&printType=books&startIndex=${startIndex}&orderBy=${orderBy}&langRestrict=${langRestrict}&key=${apiEnvs.key}`
    :
      `https://www.googleapis.com/books/v1/volumes?q=${query}&printType=books&startIndex=${startIndex}&orderBy=${orderBy}&filter=${filter}&langRestrict=${langRestrict}&key=${apiEnvs.key}`
    ;
  ...
  public getBook(id: string): Observable<BookModel> {
    ...
    // map the data to the BookModel
    return this.http.get(`https://www.googleapis.com/books/v1/volumes/${id}?key=${apiEnvs.key}`).pipe(map((book: any) => {
